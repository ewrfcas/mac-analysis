...
public class DBSCANImpl {
    ...
    @Autowired
    private transient CutImpl cutImpl;
...
//        }
    ...
    public List<DataSet> getDataSet(List<Data> datas){
        ...
        for(Data data:datas){
            for(DataDetail dataDetail:data.getDataDetails()){//去除休息室的探测器
                if(dataDetail.getDeviceId().equals("8482f42f22d8")){
                    data.getDataDetails().remove(dataDetail);
                }
            }
            ...
            double stayTime=cutImpl.getStayTime(data.getDataDetails());
